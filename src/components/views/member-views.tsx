  ...
  return <div className="animate-enter">
      ...
      <section className="surface min-w-0 rounded-xl p-4 sm:p-5">
        ...
        <div className="grid gap-5 md:grid-cols-[1fr_190px]">
          ...
          <div className="grid grid-cols-2 gap-2 md:grid-cols-1">
            <div className="rounded-lg bg-[#f8f6f2] p-3"><p className="text-[0.6875rem] text-stone-500">本月已結算</p><p className="font-data mt-1 text-lg font-semibold">{formatTwd(14820)}</p></div>
            <div className="rounded-lg bg-[#f8efd9] p-3"><p className="text-[0.6875rem] text-[#795307]">較上月</p><p className="font-data mt-1 text-lg font-semibold text-[#795307]">+18.6%</p></div>
          </div>
        </div>
      </section>
      <section className="surface min-w-0 rounded-xl p-4 sm:p-5">
    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-[.82fr_1.18fr]">
      <section className="surface min-w-0 rounded-xl p-4 sm:p-5">
        ...
        <div className="grid grid-cols-3 gap-2 text-center"><div className="rounded-lg bg-stone-50 p-3"><p className="font-data text-xl font-semibold">{member.directCount}</p><p className="text-[0.6875rem] text-stone-500">有效直推</p></div><div className="rounded-lg bg-stone-50 p-3"><p className="font-data text-xl font-semibold">{member.unlockedGenerations}</p><p className="text-[0.6875rem] text-stone-500">可領代數</p></div><div className="rounded-lg bg-stone-50 p-3"><p className="font-data text-xl font-semibold">{children.length * 9 + 12}</p><p className="text-[0.6875rem] text-stone-500">組織人數</p></div></div>
...
function ProductCard({ product, onAdd }: { product: Product; onAdd: () => void }) {
  return <article className="group overflow-hidden rounded-xl border border-[#ded9d2] bg-[#fffdfa] transition duration-200 hover:-translate-y-0.5 hover:shadow-[0_18px_40px_rgba(28,25,23,.1)]">
    <div className="relative aspect-[4/3] overflow-hidden" style={{ background: `linear-gradient(145deg, ${product.accent}26, #ebe7df)` }}><Image src={product.image} alt={product.name} fill sizes="(max-width: 640px) 100vw, (max-width: 1280px) 50vw, 25vw" className="object-cover transition duration-300 group-hover:scale-[1.03]" /><span className="absolute left-3 top-3 rounded-full bg-white/85 px-2 py-1 text-[0.625rem] font-semibold text-stone-600 backdrop-blur">{product.category}</span>{product.pointsAllowed && <span className="absolute bottom-3 right-3 rounded-full bg-[#241f1b] px-2 py-1 text-[0.625rem] font-semibold text-[#e0c47d]">點數可用</span>}</div>
    <div className="p-4"><h3 className="font-semibold tracking-[-.01em]">{product.name}</h3><p className="mt-1 text-xs text-stone-500">{product.subtitle}</p><div className="mt-4 flex items-end justify-between gap-3"><div><p className="font-data text-lg font-semibold">{formatTwd(product.priceTwd)}</p><p className="text-[0.625rem] text-stone-400">庫存 {product.stock}</p></div><button onClick={onAdd} className="grid size-11 place-items-center rounded-lg bg-[#1c1917] text-white transition hover:bg-[#8a6107]" aria-label={`加入購物車：${product.name}`}><Plus className="size-5" /></button></div></div>
  </article>;
}
