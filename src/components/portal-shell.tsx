  ...
  return (
    <div className="min-h-screen bg-[#f3f1ed]">
      ...
      <div className={`min-h-screen transition-[padding] duration-200 ${collapsed ? "lg:pl-[76px]" : "lg:pl-[244px]"}`}>
        <div className="bg-[#2a241f] px-3 py-2 text-center text-[11px] font-semibold tracking-wide text-[#ead59e]">
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-[#ded9d2] bg-[#fffdfa]/95 px-3 backdrop-blur sm:px-5">
          ...
          <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-[#292524]">{current.label}</p><p className="hidden text-[10px] text-stone-400 sm:block">LUXKEY / {roleLabels[role]} / {current.label}</p></div>
          <button className="hidden min-h-10 w-full max-w-[280px] items-center gap-2 rounded-lg border border-[#ded9d2] bg-white px-3 text-left text-sm text-stone-400 hover:border-[#b8afa5] md:flex"><Search className="size-4" />搜尋會員、訂單或流水 <kbd className="ml-auto rounded border border-stone-200 px-1.5 py-0.5 text-[10px]">⌘ K</kbd></button>
          <div className="relative">
            ...
            {noticeOpen && <div className="absolute right-0 top-12 w-[310px] rounded-xl border border-[#ded9d2] bg-[#fffdfa] p-3 shadow-xl animate-enter"><p className="px-2 py-1 text-xs font-bold tracking-wide text-stone-500">最新通知</p>{["2026 年 8 月月結批次待確認", "2 項商品等待審核", "1 則頁面標註尚未處理"].map((text, index) => <div key={text} className="mt-1 flex gap-3 rounded-lg p-2.5 hover:bg-stone-50"><span className={`mt-1.5 size-2 shrink-0 rounded-full ${index === 0 ? "bg-[#a16207]" : "bg-stone-300"}`} /><div><p className="text-sm text-stone-800">{text}</p><p className="mt-0.5 text-[10px] text-stone-400">{index + 1} 小時前</p></div></div>)}</div>}
          </div>
          <div className="relative">
            <button onClick={() => setAccountOpen((value) => !value)} className="flex min-h-11 items-center gap-2 rounded-lg px-1.5 hover:bg-stone-100" aria-expanded={accountOpen}><span className="grid size-8 place-items-center rounded-full bg-[#2d2824] text-xs font-bold text-[#e2c77f]">{account.displayName.slice(-2)}</span><span className="hidden text-left xl:block"><span className="block text-xs font-semibold text-stone-800">{account.displayName}</span><span className="block text-[10px] text-stone-400">{account.username}</span></span><ChevronDown className="hidden size-4 text-stone-400 sm:block" /></button>
